...
function matrix_to_array_of_array(mat::AbstractMatrix)
    return [mat[i,:] for i in 1:size(mat,1)]
end # function]
...
function signed_volume_of_triangle(p1::T,p2::T,p3::T) where {T <: AbstractVector}
...
"""
function mesh_volume(verts::Array{Point{3,T1}},
                    faces::Array{Face{3,T2}}) where {T1<:Real,T2<:Int}
    total_vol = 0.0
    for face in faces
        vol = signed_volume_of_triangle(verts[face[1]],verts[face[2]],verts[face[3]])
        total_vol += vol
    end
...
"""
function volume_normalizing(verts::Array{Point{3,T1}},
                    faces::Array{Face{3,T2}}) where {T1<:Real,T2<:Int}
...
function volume_normalizing2(verts::Array{Point{3,<:Real}},
                    faces::Array{Face{3,<:Int}})
    vol = mesh_volume(verts,faces)
    cbrt_vol = cbrt(vol)
    new_verts = similar(verts)
    for i in eachindex(verts)
